/* =============================================================
   daugia.vin — app.js (v7, synced to latest HTML/CSS)
   ...
   - Show wallet chip (address + VIC/VIN balances, auto-refresh)
   - Register button only when connected & NOT registered
   - Search by organizer address
   - Follow organizers (localStorage) to filter list (Theo dõi / Xóa)
   - Load & render auctions (read-only)
   ============================================================= */
...
async function refreshPlatformFee(){
  try {
    ...
    const fee = Number(ethers.utils.formatUnits(platformFeeVIN, dec));
    if (el.platformFee) el.platformFee.textContent = `${fee} VIN`;
  } catch {
    if (el.platformFee) el.platformFee.textContent = "không đọc được";
  }
}

// ---------- Registration visibility ----------
async function isRegistered(addr){
  // Kiểm tra event OrganizerRegistered(addr, …)
  try{
...
function updateFollowUI(){
  const list = getFollow();
  if (el.followHint) el.followHint.textContent = list.length ? `Đang theo dõi ${list.length} organizer` : "Chưa có ví theo dõi";
  if (el.filterPill){
    if (list.length){ el.filterPill.textContent = `Lọc: ${list.length} organizer`; setHidden(el.filterPill, false); }
    else { setHidden(el.filterPill, true); }
  }
}
function addFollowFromInput(){
  ...
  const s = new Set(getFollow());
  s.add(addr.toLowerCase());
  setFollow(Array.from(s));
  updateFollowUI();
...
async function loadAllAuctions(){
  ...
  try{
    // totalAuctions() via raw call (không cần trong ABI)
...
function applyFiltersAndRender(){
  ...
  if (hasFollow){
    items = items.filter(({a}) => (a[0]||"").toLowerCase() && follow.has(String(a[0]).toLowerCase()));
  }
